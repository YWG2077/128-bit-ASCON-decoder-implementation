...
def ascon_process_ciphertext(S, b, rate, ciphertext):
    ...
    # last block t
    if debug: printstate(S, "Now we enter the final stage:")
...
def ascon_finalize(S, rate, a, key):
    ...
    if debug: printstate(S, "Now the key has already been applied:")

    ascon_permutation(S, a)
    if debug: printstate(S, "Now the 12 iteration are terminated")
